...
for l in Lines:
    ...
    if not move_match:
        ...
        print('moving {} from {} to {}'.format(num_to_move, source, target))
        moving_pile = []
        for n in range(int(num_to_move)):
            print(crate_piles)
            char_to_move = crate_piles[int(source)].pop(0)
            moving_pile.append(char_to_move)
        for i in range(len(moving_pile)):
            crate_piles[int(target)].insert(0, moving_pile[len(moving_pile)-1-i])
